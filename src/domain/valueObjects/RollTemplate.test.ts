...
describe("RollTemplate", () => {
  ...
  describe("isTemplate", () => {
    it("should return true for strings with template references", () => {
      expect(
        RollTemplate.isTemplate(
          "You encounter a {{monster::monster-table::monsters}}"
        )
      ).toBe(true);
      expect(
        RollTemplate.isTemplate(
          "Multiple {{ref1::table1::name1}} and {{ref2::table2::name2}}"
        )
      ).toBe(true);
    });
    ...
    it("should return false for strings without template references", () => {
      ...
      expect(
        RollTemplate.isTemplate("Almost a template { but not quite }")
      ).toBe(false);
    });
  ...
  describe("replaceReference", () => {
    it("should replace a reference with a value", () => {
      ...
      const reference = new TemplateReference(
        "monster",
        "monster-table",
        "monsters"
      );
    ...
    it("should replace only the first occurrence of a reference", () => {
      ...
      expect(newTemplate.template).toBe(
        "goblin and another {{monster::monster-table::monsters}}"
      );
    });
