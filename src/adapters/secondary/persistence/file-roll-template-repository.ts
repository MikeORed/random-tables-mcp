import {
  RollTemplateEntity,
  RollTemplateEntityDTO,
} from '../../../domain/entities/roll-template-entity.js';
 ...
 */
export class FileRollTemplateRepository implements RollTemplateRepository {
  ...
  async getById(id: string): Promise<RollTemplateEntity | null> {
    ...
    try {
      ...
      const templateData = JSON.parse(data) as RollTemplateEntityDTO;
  ...
  async list(filter?: Record<string, unknown>): Promise<RollTemplateEntity[]> {
    ...
    try {
      ...
      const templateFiles = files.filter(
        file => file.startsWith('template-') && file.endsWith('.json'),
      );

      const templates: RollTemplateEntity[] = [];
