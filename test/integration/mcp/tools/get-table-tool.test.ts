...
describe('GetTableTool Integration Tests', () => {
  ...
  it('should have the correct name and description', () => {
    ...
    expect(getTableTool.getToolDefinition().description).toBe('Get a specific random table by ID');
  });
  ...
  it('should throw an error when table does not exist', async () => {
    ...
    await expect(getTableTool.execute({ tableId: nonExistentId })).rejects.toThrow(
      `Table with ID ${nonExistentId} not found`,
    );
  });
