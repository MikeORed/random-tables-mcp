...
describe('TableResource Integration Tests', () => {
  ...
  it('should throw an error when given an invalid ID', async () => {
    await expect(tableResource.getContent({ tableId: 'non-existent-id' })).rejects.toThrow(
      'Table with ID non-existent-id not found',
    );
  });
  ...
  it('should handle tables with no entries', async () => {
    ...
    const emptyTable = new RandomTable(emptyTableId, 'Empty Table', 'A table with no entries', []);
