import { FileTableRepository } from "../../../../../src/adapters/secondary/persistence/FileTableRepository";
...
describe("FileTableRepository", () => {
  ...
  describe("initialize", () => {
    ...
    it("should create data directory if it doesn't exist", async () => {
      ...
      (fs.access as unknown as jest.Mock).mockImplementationOnce(
        (path, callback) => {
          callback(new Error("ENOENT"));
        }
      );
  ...
  describe("save", () => {
    it("should save a table to a file and return its ID", async () => {
      ...
      expect(mockWriteFile).toHaveBeenCalledWith(
        `${testDataDir}/${sampleTable.id}.json`,
  ...
  describe("update", () => {
    it("should throw an error when updating a non-existent table", async () => {
      ...
      const nonExistentTable = new RandomTable(
        "non-existent-table",
        "Non-existent Table"
      );
